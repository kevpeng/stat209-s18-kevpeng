...
ggplot(teachers, aes(base)) + geom_histogram()
```

Do most of the salary values fall within the range given in question 5? Why
or why not?

**Answer**: No. I believe the reason why, is that there are many values above and below the mean rather than center around the mean. 
...
```{r}
model1 <- lm_basic(base ~ 1, data = masters)
reg_table(model1, level = 0.95)

```
 
Overall, getting a master's degree does result in higher salary. 
...
```{r}
model2 <- lm_basic(age ~ 1, data = london)
reg_table(model2, level = 0.80)
```
...
**Answer**: There is an 80% chance that the true mean lies between 33.07 and 34.65.
