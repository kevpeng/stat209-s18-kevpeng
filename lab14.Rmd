...
Create a new dataset which summarizes everything to the level of a bird type using the `group_summarize` function:

```{r}
level_type <- group_summarize(birds, type)
```
...
```{r}
ggplot(level_type, aes(egg_mass_median, male_mass_median)) + 
  geom_point() +
  scale_x_log10() +
  scale_y_log10() 
``` 
...
```{r}
n_5_birds <- filter(level_type, n >= 5)
...
ggplot(level_type, aes(egg_mass_median, male_mass_median)) + 
  geom_point() +
  geom_point(data=n_5_birds, color = "red") +
  scale_x_log10() +
  scale_y_log10() +
  # this is for the "key"
  annotate("point", x = 2.25, y = 900,
     color = "red") +
  annotate("rect", xmin = 0.75, xmax = 6.75,
     ymin = 700, ymax = 1600,
     fill = "purple", alpha = 0.1) +
  annotate("text", x = 2.25, y = 1400,
           label = "More than 5 or more bird types")
```
